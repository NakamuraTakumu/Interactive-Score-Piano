...
| tuplet / triplet | OSMD が `<time-modification>` / `<tuplet>` を `sourceNote.Length` に正規化した結果を読む。確認譜面では三連符 3 音が各 `1/12`、`PPQ=480` で各 160 tick になった。 | **OSMD 依存で部分対応**。tuplet 記号の表示は OSMD が担い、アプリ側で tuplet ratio や bracket を直接解釈しない。 |
...
- `src/App.tsx`: scheduler は tick 昇順の note-on / note-off を進め、tempo map と速度倍率から現在 tick を計算する。repeat 展開、instrument map は持たない。
...
現在の再生機能は、MusicXML を「演奏データ」として網羅的に解釈する実装ではなく、「表示済み譜面から単純な note-on / note-off list と数値 tempo map を作る」実装である。したがって、対応範囲を数値化するなら、MusicXML playback semantics のうち **直線的な pitched notes / rests / chords / basic voices / numeric tempo map / tie chain / 一部 articulation の単純変形**だけを対象にする段階で、反復、多くの音符装飾、fermata、奏法、楽器、percussion を含む実演奏仕様の大半は未対応と見るのが妥当である。
